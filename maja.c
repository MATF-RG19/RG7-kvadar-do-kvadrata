...
int n, m, p, q, indikator=0, up_key=0, key_pre=0, ind_pre=0, count_3=0, count_4=0, 
    ...
    /*Vreme trajanja igrice, početno vreme i vremenska razlika.*/
GLfloat countdown=0, begin_time, diff_time;
...
float rotation;
    ...
    /* Stringovna reprezentacija broja poteza i proteklog vremena. */
char buffer[20];
char time_buffer[20];
...
void on_display(void)
{
    ...
    sprintf(time_buffer, "Time: %.1lf", countdown);
